...
<?php include_once "Job_Position_Vs_Number_of_Interviewsctbinfo.php" ?>
<?php
...
$Job_Position_Vs_Number_of_Interviews_crosstab = NULL; // Initialize page object first

class crJob_Position_Vs_Number_of_Interviews_crosstab extends crJob_Position_Vs_Number_of_Interviews {
	...
	// Page object name
	var $PageObjName = 'Job_Position_Vs_Number_of_Interviews_crosstab';
	...
	function __construct() {
		...
		// Table object (Job_Position_Vs_Number_of_Interviews)
		if (!isset($GLOBALS["Job_Position_Vs_Number_of_Interviews"])) {
			$GLOBALS["Job_Position_Vs_Number_of_Interviews"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Job_Position_Vs_Number_of_Interviews"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Job Position Vs Number of Interviews', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fJob_Position_Vs_Number_of_Interviewscrosstab";
	}
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fJob_Position_Vs_Number_of_Interviewscrosstab\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fJob_Position_Vs_Number_of_Interviewscrosstab\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fJob_Position_Vs_Number_of_Interviewscrosstab\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$_SESSION["sel_Job_Position_Vs_Number_of_Interviews_YEAR__dateCreated"] = "";
				$this->ResetPager();
			}
		}
		...
		if (@$_GET["YEAR__dateCreated"] <> "") {
			...
			if (is_numeric($this->YEAR__dateCreated->QueryStringValue)) {
				$_SESSION["sel_Job_Position_Vs_Number_of_Interviews_YEAR__dateCreated"] = $this->YEAR__dateCreated->QueryStringValue;
				$this->ResetPager();
			}
		}
		$this->YEAR__dateCreated->SelectionList = @$_SESSION["sel_Job_Position_Vs_Number_of_Interviews_YEAR__dateCreated"];
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			for ($i = 0; $i < $scvcnt; $i++) {
				...
				$this->Cell_Rendered($this->Number_Of_Interviews, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
			}
		} else {
	...
	function CheckFilter() {
		...
		// Year Filter
		if (@$_SESSION["sel_Job_Position_Vs_Number_of_Interviews_YEAR__dateCreated"] <> "")
			return TRUE;
		return FALSE;
	}
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_YEAR__dateCreated", $filter)) {
			...
			$_SESSION["sel_Job_Position_Vs_Number_of_Interviews_YEAR__dateCreated"] = $ar;
		}
	}
...
// Create page object
if (!isset($Job_Position_Vs_Number_of_Interviews_crosstab)) $Job_Position_Vs_Number_of_Interviews_crosstab = new crJob_Position_Vs_Number_of_Interviews_crosstab();
if (isset($Page)) $OldPage = $Page;
$Page = &$Job_Position_Vs_Number_of_Interviews_crosstab;
...
// Create page object
var Job_Position_Vs_Number_of_Interviews_crosstab = new ewr_Page("Job_Position_Vs_Number_of_Interviews_crosstab");

// Page properties
Job_Position_Vs_Number_of_Interviews_crosstab.PageID = "crosstab"; // Page ID
var EWR_PAGE_ID = Job_Position_Vs_Number_of_Interviews_crosstab.PageID;

// Extend page with Chart_Rendering function
Job_Position_Vs_Number_of_Interviews_crosstab.Chart_Rendering = 
...
Job_Position_Vs_Number_of_Interviews_crosstab.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var fJob_Position_Vs_Number_of_Interviewscrosstab = new ewr_Form("fJob_Position_Vs_Number_of_Interviewscrosstab");

// Validate method
fJob_Position_Vs_Number_of_Interviewscrosstab.Validate = function() {
...
fJob_Position_Vs_Number_of_Interviewscrosstab.Form_CustomValidate = 
...
fJob_Position_Vs_Number_of_Interviewscrosstab.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fJob_Position_Vs_Number_of_Interviewscrosstab.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fJob_Position_Vs_Number_of_Interviewscrosstab" id="fJob_Position_Vs_Number_of_Interviewscrosstab" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fJob_Position_Vs_Number_of_Interviewscrosstab_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<div id="c_YEAR__dateCreated" class="ewCell form-group">
	...
	<select id="YEAR__dateCreated" class="form-control" name="YEAR__dateCreated" onchange="ewrForms['fJob_Position_Vs_Number_of_Interviewscrosstab'].Submit();">
<?php
...
<script type="text/javascript">
fJob_Position_Vs_Number_of_Interviewscrosstab.Init();
fJob_Position_Vs_Number_of_Interviewscrosstab.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
<?php } ?>
...
<?php include "Job_Position_Vs_Number_of_Interviewsctbpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php if ($Page->GrpFldCount > 0) { ?>
		<td class="ewRptColSummary" colspan="<?php echo $Page->GrpFldCount ?>"><div><?php echo $Page->Number_Of_Interviews->FldCaption() ?>&nbsp;(<?php echo $ReportLanguage->Phrase("RptSum") ?>)&nbsp;</div></td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="jbName">
		<div class="Job_Position_Vs_Number_of_Interviews_jbName"><span class="ewTableHeaderCaption"><?php echo $Page->jbName->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="jbName">
<?php if ($Page->SortUrl($Page->jbName) == "") { ?>
		<div class="ewTableHeaderBtn ewPointer Job_Position_Vs_Number_of_Interviews_jbName" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->jbName) ?>',0);">
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_Job_Position_Vs_Number_of_Interviews_jbName"<?php echo $Page->jbName->ViewAttributes() ?>><?php echo $Page->jbName->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
...
<?php } ?>
	<!-- Bottom slot -->
<a id="cht_Job_Position_Vs_Number_of_Interviews_Job_Positions_Vs_Number_of_Interviews"></a>
<div class="">
<div id="div_ctl_Job_Position_Vs_Number_of_Interviews_Job_Positions_Vs_Number_of_Interviews" class="ewChart">
<div id="div_Job_Position_Vs_Number_of_Interviews_Job_Positions_Vs_Number_of_Interviews" class="ewChartDiv"></div>
<!-- grid component -->
<div id="div_Job_Position_Vs_Number_of_Interviews_Job_Positions_Vs_Number_of_Interviews_grid" class="ewChartGrid"></div>
</div>
</div>
<?php
...
?>
<?php include_once "Job_Position_Vs_Number_of_Interviews_Job_Positions_Vs_Number_of_Interviewschart.php" ?>
