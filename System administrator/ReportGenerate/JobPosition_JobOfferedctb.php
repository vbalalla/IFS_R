...
<?php include_once "JobPosition_JobOfferedctbinfo.php" ?>
<?php
...
$JobPosition_JobOffered_crosstab = NULL; // Initialize page object first

class crJobPosition_JobOffered_crosstab extends crJobPosition_JobOffered {
	...
	// Page object name
	var $PageObjName = 'JobPosition_JobOffered_crosstab';
	...
	function __construct() {
		...
		// Table object (JobPosition_JobOffered)
		if (!isset($GLOBALS["JobPosition_JobOffered"])) {
			$GLOBALS["JobPosition_JobOffered"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["JobPosition_JobOffered"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'JobPosition_JobOffered', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fJobPosition_JobOfferedcrosstab";
	}
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_JobPosition_JobOffered\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_JobPosition_JobOffered',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fJobPosition_JobOfferedcrosstab\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fJobPosition_JobOfferedcrosstab\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fJobPosition_JobOfferedcrosstab\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$_SESSION["sel_JobPosition_JobOffered_YEAR__dateCreated"] = "";
				$this->ResetPager();
			}
		}
		...
		if (@$_GET["YEAR__dateCreated"] <> "") {
			...
			if (is_numeric($this->YEAR__dateCreated->QueryStringValue)) {
				$_SESSION["sel_JobPosition_JobOffered_YEAR__dateCreated"] = $this->YEAR__dateCreated->QueryStringValue;
				$this->ResetPager();
			}
		}
		$this->YEAR__dateCreated->SelectionList = @$_SESSION["sel_JobPosition_JobOffered_YEAR__dateCreated"];
	...
	function CheckFilter() {
		...
		// Year Filter
		if (@$_SESSION["sel_JobPosition_JobOffered_YEAR__dateCreated"] <> "")
			return TRUE;
		return FALSE;
	}
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_YEAR__dateCreated", $filter)) {
			...
			$_SESSION["sel_JobPosition_JobOffered_YEAR__dateCreated"] = $ar;
		}
	}
...
// Create page object
if (!isset($JobPosition_JobOffered_crosstab)) $JobPosition_JobOffered_crosstab = new crJobPosition_JobOffered_crosstab();
if (isset($Page)) $OldPage = $Page;
$Page = &$JobPosition_JobOffered_crosstab;
...
// Create page object
var JobPosition_JobOffered_crosstab = new ewr_Page("JobPosition_JobOffered_crosstab");

// Page properties
JobPosition_JobOffered_crosstab.PageID = "crosstab"; // Page ID
var EWR_PAGE_ID = JobPosition_JobOffered_crosstab.PageID;

// Extend page with Chart_Rendering function
JobPosition_JobOffered_crosstab.Chart_Rendering = 
...
JobPosition_JobOffered_crosstab.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var fJobPosition_JobOfferedcrosstab = new ewr_Form("fJobPosition_JobOfferedcrosstab");

// Validate method
fJobPosition_JobOfferedcrosstab.Validate = function() {
...
fJobPosition_JobOfferedcrosstab.Form_CustomValidate = 
...
fJobPosition_JobOfferedcrosstab.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fJobPosition_JobOfferedcrosstab.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fJobPosition_JobOfferedcrosstab" id="fJobPosition_JobOfferedcrosstab" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fJobPosition_JobOfferedcrosstab_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<div id="c_YEAR__dateCreated" class="ewCell form-group">
	...
	<select id="YEAR__dateCreated" class="form-control" name="YEAR__dateCreated" onchange="ewrForms['fJobPosition_JobOfferedcrosstab'].Submit();">
<?php
...
<script type="text/javascript">
fJobPosition_JobOfferedcrosstab.Init();
fJobPosition_JobOfferedcrosstab.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
<?php } ?>
...
<?php include "JobPosition_JobOfferedctbpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Job_Name">
		<div class="JobPosition_JobOffered_Job_Name"><span class="ewTableHeaderCaption"><?php echo $Page->Job_Name->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="Job_Name">
<?php if ($Page->SortUrl($Page->Job_Name) == "") { ?>
		<div class="ewTableHeaderBtn ewPointer JobPosition_JobOffered_Job_Name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Job_Name) ?>',0);">
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_JobPosition_JobOffered_Job_Name"<?php echo $Page->Job_Name->ViewAttributes() ?>><?php echo $Page->Job_Name->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
...
<?php } ?>
	<!-- Right slot -->
<a id="cht_JobPosition_JobOffered_C_job_position_vs_job_offered"></a>
<div class="">
<div id="div_ctl_JobPosition_JobOffered_C_job_position_vs_job_offered" class="ewChart">
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<div class="ewChartSort">
<form class="ewForm form-horizontal" action="<?php echo ewr_CurrentPage() ?>#cht_JobPosition_JobOffered_C_job_position_vs_job_offered">
...
<?php } ?>
<div id="div_JobPosition_JobOffered_C_job_position_vs_job_offered" class="ewChartDiv"></div>
<!-- grid component -->
<div id="div_JobPosition_JobOffered_C_job_position_vs_job_offered_grid" class="ewChartGrid"></div>
</div>
</div>
<?php
...
?>
<?php include_once "JobPosition_JobOffered_C_job_position_vs_job_offeredchart.php" ?>
<?php if ($Page->Export == "") { ?>
	</div>
	<!-- right container (end) -->
<div class="clearfix"></div>
