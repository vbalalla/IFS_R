...
while($row = $criteria->fetch_row()){
	...
	//echo "UPDATE $table SET $row[0]=$value WHERE CandID='$cid'";
		
	mysqli_query($connect, "UPDATE $table SET $row[0]=$value WHERE CandID='$cid' AND panelMember='E002'");
}

$a=$_POST['comments'];
echo $a;
mysqli_query($connect, "UPDATE $table SET comment='$a' WHERE CandID='$cid' AND panelMember='E002'");
