...
<?php include_once "Job_Offered_Candidates_Vs_Universityctbinfo.php" ?>
<?php
...
$Job_Offered_Candidates_Vs_University_crosstab = NULL; // Initialize page object first

class crJob_Offered_Candidates_Vs_University_crosstab extends crJob_Offered_Candidates_Vs_University {
	...
	// Page object name
	var $PageObjName = 'Job_Offered_Candidates_Vs_University_crosstab';
	...
	function __construct() {
		...
		// Table object (Job_Offered_Candidates_Vs_University)
		if (!isset($GLOBALS["Job_Offered_Candidates_Vs_University"])) {
			$GLOBALS["Job_Offered_Candidates_Vs_University"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Job_Offered_Candidates_Vs_University"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Job Offered Candidates Vs University', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fJob_Offered_Candidates_Vs_Universitycrosstab";
	}
	...
	function Page_Init() {
		...
		// Setup placeholder
		$this->University->PlaceHolder = $this->University->FldCaption();
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_Job_Offered_Candidates_Vs_University\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Job_Offered_Candidates_Vs_University',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fJob_Offered_Candidates_Vs_Universitycrosstab\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fJob_Offered_Candidates_Vs_Universitycrosstab\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fJob_Offered_Candidates_Vs_Universitycrosstab\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->University->SelectionList = "";
		$this->University->DefaultSelectionList = "";
		$this->University->ValueList = "";
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			// Check popup name
			// Build distinct values for University

			if ($popupname == 'Job_Offered_Candidates_Vs_University_University') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
				$sSql = ewr_BuildReportSql($this->University->SqlSelect, $this->getSqlWhere(), $this->getSqlGroupBy(), $this->getSqlHaving(), $this->University->SqlOrderBy, $sFilter, "");
				$rswrk = $conn->Execute($sSql);
				while ($rswrk && !$rswrk->EOF) {
					$this->University->setDbValue($rswrk->fields[0]);
					if (is_null($this->University->CurrentValue)) {
						$bNullValue = TRUE;
					} elseif ($this->University->CurrentValue == "") {
						$bEmptyValue = TRUE;
					} else {
						$this->University->GroupViewValue = $this->University->GroupValue();
						ewr_SetupDistinctValues($this->University->ValueList, $this->University->GroupValue(), $this->University->GroupViewValue, FALSE);
					}
					$rswrk->MoveNext();
				}
				if ($rswrk)
					$rswrk->Close();
				if ($bEmptyValue)
					ewr_SetupDistinctValues($this->University->ValueList, EWR_EMPTY_VALUE, $ReportLanguage->Phrase("EmptyLabel"), FALSE);
				if ($bNullValue)
					ewr_SetupDistinctValues($this->University->ValueList, EWR_NULL_VALUE, $ReportLanguage->Phrase("NullLabel"), FALSE);
				$fld = &$this->University;
			}
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$_SESSION["sel_Job_Offered_Candidates_Vs_University_YEAR__dateCreated"] = "";
				$this->ClearSessionSelection('University');
				$this->ResetPager();
			}
		}

		// Load selection criteria to array
		// Get University selected values

		if (is_array(@$_SESSION["sel_Job_Offered_Candidates_Vs_University_University"])) {
			$this->LoadSelectionFromSession('University');
		} elseif (@$_SESSION["sel_Job_Offered_Candidates_Vs_University_University"] == EWR_INIT_VALUE) { // Select all
			$this->University->SelectionList = "";
		}
		...
		if (@$_GET["YEAR__dateCreated"] <> "") {
			...
			if (is_numeric($this->YEAR__dateCreated->QueryStringValue)) {
				$_SESSION["sel_Job_Offered_Candidates_Vs_University_YEAR__dateCreated"] = $this->YEAR__dateCreated->QueryStringValue;
				$this->ResetPager();
			}
		}
		$this->YEAR__dateCreated->SelectionList = @$_SESSION["sel_Job_Offered_Candidates_Vs_University_YEAR__dateCreated"];
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			// Clear extended filter for field University
			if ($this->ClearExtFilter == 'Job_Offered_Candidates_Vs_University_University')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'University');

		// Reset search command
		} elseif (@$_GET["cmd"] == "reset") {

			// Load default values
			$this->SetSessionFilterValues($this->University->SearchValue, $this->University->SearchOperator, $this->University->SearchCondition, $this->University->SearchValue2, $this->University->SearchOperator2, 'University'); // Field University
			...
			// Field University
			if ($this->GetFilterValues($this->University)) {
				$bSetupFilter = TRUE;
			}
		...
		// Restore session
		if ($bRestoreSession) {
			$this->GetSessionFilterValues($this->University); // Field University
		}
		...
		// Build SQL
		$this->BuildExtendedFilter($this->University, $sFilter, FALSE, TRUE); // Field University
		...
		if ($bSetupFilter) {
			...
			// Field University
			$sWrk = "";
			$this->BuildExtendedFilter($this->University, $sWrk);
			ewr_LoadSelectionFromFilter($this->University, $sWrk, $this->University->SelectionList);
			$_SESSION['sel_Job_Offered_Candidates_Vs_University_University'] = ($this->University->SelectionList == "") ? EWR_INIT_VALUE : $this->University->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_Job_Offered_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Job_Offered_Candidates_Vs_University_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Job_Offered_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Job_Offered_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_Job_Offered_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_Job_Offered_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_Job_Offered_Candidates_Vs_University_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_Job_Offered_Candidates_Vs_University_' . $parm] = $sv;
		$_SESSION['so_Job_Offered_Candidates_Vs_University_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Job_Offered_Candidates_Vs_University_' . $parm] = $sv1;
		$_SESSION['so_Job_Offered_Candidates_Vs_University_' . $parm] = $so1;
		$_SESSION['sc_Job_Offered_Candidates_Vs_University_' . $parm] = $sc;
		$_SESSION['sv2_Job_Offered_Candidates_Vs_University_' . $parm] = $sv2;
		$_SESSION['so2_Job_Offered_Candidates_Vs_University_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Job_Offered_Candidates_Vs_University_$parm"] = "";
		$_SESSION["rf_Job_Offered_Candidates_Vs_University_$parm"] = "";
		$_SESSION["rt_Job_Offered_Candidates_Vs_University_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_Job_Offered_Candidates_Vs_University_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Job_Offered_Candidates_Vs_University_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Job_Offered_Candidates_Vs_University_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field University
		$this->SetDefaultExtFilter($this->University, "LIKE", NULL, 'AND', "=", NULL);
		if (!$this->SearchCommand) $this->ApplyDefaultExtFilter($this->University);
		$sWrk = "";
		$this->BuildExtendedFilter($this->University, $sWrk, TRUE);
		ewr_LoadSelectionFromFilter($this->University, $sWrk, $this->University->DefaultSelectionList);
		if (!$this->SearchCommand) $this->University->SelectionList = $this->University->DefaultSelectionList;
		...
		*/
		...
		// Field University
		// $this->University->DefaultSelectionList = array("val1", "val2");
	...
	function CheckFilter() {
		...
		// Year Filter
		if (@$_SESSION["sel_Job_Offered_Candidates_Vs_University_YEAR__dateCreated"] <> "")
			return TRUE;

		// Check University text filter
		if ($this->TextFilterApplied($this->University))
			return TRUE;

		// Check University popup filter
		if (!ewr_MatchedArray($this->University->DefaultSelectionList, $this->University->SelectionList))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList() {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildExtendedFilter($this->University, $sExtWrk);
		if (is_array($this->University->SelectionList))
			$sWrk = ewr_JoinArray($this->University->SelectionList, ", ", EWR_DATATYPE_STRING, 0, $this->DBID);
		$sFilter = "";
		if ($sExtWrk <> "")
		...
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->University->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field University
		$sWrk = "";
		if ($this->University->SearchValue <> "" || $this->University->SearchValue2 <> "") {
			$sWrk = "\"sv_University\":\"" . ewr_JsEncode2($this->University->SearchValue) . "\"," .
				"\"so_University\":\"" . ewr_JsEncode2($this->University->SearchOperator) . "\"," .
				"\"sc_University\":\"" . ewr_JsEncode2($this->University->SearchCondition) . "\"," .
				"\"sv2_University\":\"" . ewr_JsEncode2($this->University->SearchValue2) . "\"," .
				"\"so2_University\":\"" . ewr_JsEncode2($this->University->SearchOperator2) . "\"";
		}
		if ($sWrk == "") {
			$sWrk = ($this->University->SelectionList <> EWR_INIT_VALUE) ? $this->University->SelectionList : "";
			if (is_array($sWrk))
				$sWrk = implode("||", $sWrk);
			if ($sWrk <> "")
				$sWrk = "\"sel_University\":\"" . ewr_JsEncode2($sWrk) . "\"";
		}
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_YEAR__dateCreated", $filter)) {
			...
			$_SESSION["sel_Job_Offered_Candidates_Vs_University_YEAR__dateCreated"] = $ar;
		}

		// Field University
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_University", $filter) || array_key_exists("so_University", $filter) ||
			array_key_exists("sc_University", $filter) ||
			array_key_exists("sv2_University", $filter) || array_key_exists("so2_University", $filter)) {
			$this->SetSessionFilterValues(@$filter["sv_University"], @$filter["so_University"], @$filter["sc_University"], @$filter["sv2_University"], @$filter["so2_University"], "University");
			$bRestoreFilter = TRUE;
		}
		if (array_key_exists("sel_University", $filter)) {
			$sWrk = $filter["sel_University"];
			$sWrk = explode("||", $sWrk);
			$this->University->SelectionList = $sWrk;
			$_SESSION["sel_Job_Offered_Candidates_Vs_University_University"] = $sWrk;
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "University"); // Clear extended filter
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "University");
			$this->University->SelectionList = "";
			$_SESSION["sel_Job_Offered_Candidates_Vs_University_University"] = "";
		}
	}
	...
	function GetPopupFilter() {
		...
		if ($this->DrillDown)
			return "";
		if (!$this->ExtendedFilterExist($this->University)) {
			if (is_array($this->University->SelectionList)) {
				$sFilter = ewr_FilterSQL($this->University, "`University`", EWR_DATATYPE_STRING, $this->DBID);

				// Call Page Filtering event
				$this->Page_Filtering($this->University, $sFilter, "popup");
				$this->University->CurrentFilter = $sFilter;
...
// Create page object
if (!isset($Job_Offered_Candidates_Vs_University_crosstab)) $Job_Offered_Candidates_Vs_University_crosstab = new crJob_Offered_Candidates_Vs_University_crosstab();
if (isset($Page)) $OldPage = $Page;
$Page = &$Job_Offered_Candidates_Vs_University_crosstab;
...
// Create page object
var Job_Offered_Candidates_Vs_University_crosstab = new ewr_Page("Job_Offered_Candidates_Vs_University_crosstab");

// Page properties
Job_Offered_Candidates_Vs_University_crosstab.PageID = "crosstab"; // Page ID
var EWR_PAGE_ID = Job_Offered_Candidates_Vs_University_crosstab.PageID;

// Extend page with Chart_Rendering function
Job_Offered_Candidates_Vs_University_crosstab.Chart_Rendering = 
...
Job_Offered_Candidates_Vs_University_crosstab.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fJob_Offered_Candidates_Vs_Universitycrosstab = new ewr_Form("fJob_Offered_Candidates_Vs_Universitycrosstab");

// Validate method
fJob_Offered_Candidates_Vs_Universitycrosstab.Validate = function() {
...
fJob_Offered_Candidates_Vs_Universitycrosstab.Form_CustomValidate = 
...
fJob_Offered_Candidates_Vs_Universitycrosstab.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fJob_Offered_Candidates_Vs_Universitycrosstab.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fJob_Offered_Candidates_Vs_Universitycrosstab" id="fJob_Offered_Candidates_Vs_Universitycrosstab" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fJob_Offered_Candidates_Vs_Universitycrosstab_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<div id="c_University" class="ewCell form-group">
	<label for="sv_University" class="ewSearchCaption ewLabel"><?php echo $Page->University->FldCaption() ?></label>
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("LIKE"); ?><input type="hidden" name="so_University" id="so_University" value="LIKE"></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->University->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="Job_Offered_Candidates_Vs_University" data-field="x_University" id="sv_University" name="sv_University" size="30" maxlength="70" placeholder="<?php echo $Page->University->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->University->SearchValue) ?>"<?php echo $Page->University->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fJob_Offered_Candidates_Vs_Universitycrosstab.Init();
fJob_Offered_Candidates_Vs_Universitycrosstab.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "Job_Offered_Candidates_Vs_Universityctbpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
</div>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="University">
		<div class="Job_Offered_Candidates_Vs_University_University"><span class="ewTableHeaderCaption"><?php echo $Page->University->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="University">
<?php if ($Page->SortUrl($Page->University) == "") { ?>
		<div class="ewTableHeaderBtn ewPointer Job_Offered_Candidates_Vs_University_University" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->University) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->University->FldCaption() ?></span>			
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Job_Offered_Candidates_Vs_University_University', false, '<?php echo $Page->University->RangeFrom; ?>', '<?php echo $Page->University->RangeTo; ?>');" id="x_University"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Job_Offered_Candidates_Vs_University_University" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->University) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->University->FldCaption() ?></span>
...
<?php
	...
	// Build detail SQL
	$sWhere = ewr_DetailFilterSQL($Page->University, $Page->getSqlFirstGroupField(), $Page->University->GroupValue(), $Page->DBID);
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_Job_Offered_Candidates_Vs_University_University"<?php echo $Page->University->ViewAttributes() ?>><?php echo $Page->University->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
...
<?php } ?>
	<!-- Bottom slot -->
<a id="cht_Job_Offered_Candidates_Vs_University_Job_Offered_Candidates_Vs_University"></a>
<div class="">
<div id="div_ctl_Job_Offered_Candidates_Vs_University_Job_Offered_Candidates_Vs_University" class="ewChart">
<div id="div_Job_Offered_Candidates_Vs_University_Job_Offered_Candidates_Vs_University" class="ewChartDiv"></div>
<!-- grid component -->
<div id="div_Job_Offered_Candidates_Vs_University_Job_Offered_Candidates_Vs_University_grid" class="ewChartGrid"></div>
</div>
</div>
<?php

// Set up chart object
$Chart = &$Table->Job_Offered_Candidates_Vs_University;
...
?>
<?php include_once "Job_Offered_Candidates_Vs_University_Job_Offered_Candidates_Vs_Universitychart.php" ?>
