...
<?php include_once "Rejected_Candidates_Vs_Universityctbinfo.php" ?>
<?php
...
$Rejected_Candidates_Vs_University_crosstab = NULL; // Initialize page object first

class crRejected_Candidates_Vs_University_crosstab extends crRejected_Candidates_Vs_University {
	...
	// Page object name
	var $PageObjName = 'Rejected_Candidates_Vs_University_crosstab';
	...
	function __construct() {
		...
		// Table object (Rejected_Candidates_Vs_University)
		if (!isset($GLOBALS["Rejected_Candidates_Vs_University"])) {
			$GLOBALS["Rejected_Candidates_Vs_University"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Rejected_Candidates_Vs_University"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Rejected Candidates Vs University', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fRejected_Candidates_Vs_Universitycrosstab";
	}
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_Rejected_Candidates_Vs_University\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Rejected_Candidates_Vs_University',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fRejected_Candidates_Vs_Universitycrosstab\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fRejected_Candidates_Vs_Universitycrosstab\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fRejected_Candidates_Vs_Universitycrosstab\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'Rejected_Candidates_Vs_University_University') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$_SESSION["sel_Rejected_Candidates_Vs_University_YEAR__dateCreated"] = "";
		...
		if (is_array(@$_SESSION["sel_Rejected_Candidates_Vs_University_University"])) {
			$this->LoadSelectionFromSession('University');
		} elseif (@$_SESSION["sel_Rejected_Candidates_Vs_University_University"] == EWR_INIT_VALUE) { // Select all
			$this->University->SelectionList = "";
		}
		...
		if (@$_GET["YEAR__dateCreated"] <> "") {
			...
			if (is_numeric($this->YEAR__dateCreated->QueryStringValue)) {
				$_SESSION["sel_Rejected_Candidates_Vs_University_YEAR__dateCreated"] = $this->YEAR__dateCreated->QueryStringValue;
				$this->ResetPager();
			}
		}
		$this->YEAR__dateCreated->SelectionList = @$_SESSION["sel_Rejected_Candidates_Vs_University_YEAR__dateCreated"];
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'Rejected_Candidates_Vs_University_University')
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_Rejected_Candidates_Vs_University_University'] = ($this->University->SelectionList == "") ? EWR_INIT_VALUE : $this->University->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_Rejected_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Rejected_Candidates_Vs_University_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Rejected_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Rejected_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_Rejected_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_Rejected_Candidates_Vs_University_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_Rejected_Candidates_Vs_University_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_Rejected_Candidates_Vs_University_' . $parm] = $sv;
		$_SESSION['so_Rejected_Candidates_Vs_University_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Rejected_Candidates_Vs_University_' . $parm] = $sv1;
		$_SESSION['so_Rejected_Candidates_Vs_University_' . $parm] = $so1;
		$_SESSION['sc_Rejected_Candidates_Vs_University_' . $parm] = $sc;
		$_SESSION['sv2_Rejected_Candidates_Vs_University_' . $parm] = $sv2;
		$_SESSION['so2_Rejected_Candidates_Vs_University_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Rejected_Candidates_Vs_University_$parm"] = "";
		$_SESSION["rf_Rejected_Candidates_Vs_University_$parm"] = "";
		$_SESSION["rt_Rejected_Candidates_Vs_University_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_Rejected_Candidates_Vs_University_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Rejected_Candidates_Vs_University_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Rejected_Candidates_Vs_University_$parm"];
	}
	...
	function CheckFilter() {
		...
		// Year Filter
		if (@$_SESSION["sel_Rejected_Candidates_Vs_University_YEAR__dateCreated"] <> "")
			return TRUE;
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_YEAR__dateCreated", $filter)) {
			...
			$_SESSION["sel_Rejected_Candidates_Vs_University_YEAR__dateCreated"] = $ar;
		}
		...
		if (array_key_exists("sel_University", $filter)) {
			...
			$_SESSION["sel_Rejected_Candidates_Vs_University_University"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "University");
			$this->University->SelectionList = "";
			$_SESSION["sel_Rejected_Candidates_Vs_University_University"] = "";
		}
	}
...
// Create page object
if (!isset($Rejected_Candidates_Vs_University_crosstab)) $Rejected_Candidates_Vs_University_crosstab = new crRejected_Candidates_Vs_University_crosstab();
if (isset($Page)) $OldPage = $Page;
$Page = &$Rejected_Candidates_Vs_University_crosstab;
...
// Create page object
var Rejected_Candidates_Vs_University_crosstab = new ewr_Page("Rejected_Candidates_Vs_University_crosstab");

// Page properties
Rejected_Candidates_Vs_University_crosstab.PageID = "crosstab"; // Page ID
var EWR_PAGE_ID = Rejected_Candidates_Vs_University_crosstab.PageID;

// Extend page with Chart_Rendering function
Rejected_Candidates_Vs_University_crosstab.Chart_Rendering = 
...
Rejected_Candidates_Vs_University_crosstab.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fRejected_Candidates_Vs_Universitycrosstab = new ewr_Form("fRejected_Candidates_Vs_Universitycrosstab");

// Validate method
fRejected_Candidates_Vs_Universitycrosstab.Validate = function() {
...
fRejected_Candidates_Vs_Universitycrosstab.Form_CustomValidate = 
...
fRejected_Candidates_Vs_Universitycrosstab.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fRejected_Candidates_Vs_Universitycrosstab.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fRejected_Candidates_Vs_Universitycrosstab" id="fRejected_Candidates_Vs_Universitycrosstab" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fRejected_Candidates_Vs_Universitycrosstab_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="Rejected_Candidates_Vs_University" data-field="x_University" id="sv_University" name="sv_University" size="30" maxlength="70" placeholder="<?php echo $Page->University->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->University->SearchValue) ?>"<?php echo $Page->University->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fRejected_Candidates_Vs_Universitycrosstab.Init();
fRejected_Candidates_Vs_Universitycrosstab.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "Rejected_Candidates_Vs_Universityctbpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
</div>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="University">
		<div class="Rejected_Candidates_Vs_University_University"><span class="ewTableHeaderCaption"><?php echo $Page->University->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="University">
<?php if ($Page->SortUrl($Page->University) == "") { ?>
		<div class="ewTableHeaderBtn ewPointer Rejected_Candidates_Vs_University_University" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->University) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->University->FldCaption() ?></span>			
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Rejected_Candidates_Vs_University_University', false, '<?php echo $Page->University->RangeFrom; ?>', '<?php echo $Page->University->RangeTo; ?>');" id="x_University"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Rejected_Candidates_Vs_University_University" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->University) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->University->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_Rejected_Candidates_Vs_University_University"<?php echo $Page->University->ViewAttributes() ?>><?php echo $Page->University->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
...
<?php } ?>
	<!-- Bottom slot -->
<a id="cht_Rejected_Candidates_Vs_University_Rejected_Candidates_Vs_University"></a>
<div class="">
<div id="div_ctl_Rejected_Candidates_Vs_University_Rejected_Candidates_Vs_University" class="ewChart">
<div id="div_Rejected_Candidates_Vs_University_Rejected_Candidates_Vs_University" class="ewChartDiv"></div>
<!-- grid component -->
<div id="div_Rejected_Candidates_Vs_University_Rejected_Candidates_Vs_University_grid" class="ewChartGrid"></div>
</div>
</div>
<?php

// Set up chart object
$Chart = &$Table->Rejected_Candidates_Vs_University;
...
?>
<?php include_once "Rejected_Candidates_Vs_University_Rejected_Candidates_Vs_Universitychart.php" ?>
