...
<?php include_once "JobPosition_Vs_JobOffered_Candidatesctbinfo.php" ?>
<?php
...
$JobPosition_Vs_JobOffered_Candidates_crosstab = NULL; // Initialize page object first

class crJobPosition_Vs_JobOffered_Candidates_crosstab extends crJobPosition_Vs_JobOffered_Candidates {
	...
	// Page object name
	var $PageObjName = 'JobPosition_Vs_JobOffered_Candidates_crosstab';
	...
	function __construct() {
		...
		// Table object (JobPosition_Vs_JobOffered_Candidates)
		if (!isset($GLOBALS["JobPosition_Vs_JobOffered_Candidates"])) {
			$GLOBALS["JobPosition_Vs_JobOffered_Candidates"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["JobPosition_Vs_JobOffered_Candidates"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'JobPosition Vs JobOffered Candidates', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fJobPosition_Vs_JobOffered_Candidatescrosstab";
	}
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fJobPosition_Vs_JobOffered_Candidatescrosstab\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fJobPosition_Vs_JobOffered_Candidatescrosstab\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fJobPosition_Vs_JobOffered_Candidatescrosstab\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$_SESSION["sel_JobPosition_Vs_JobOffered_Candidates_YEAR__dateCreated"] = "";
				$this->ResetPager();
			}
		}
		...
		if (@$_GET["YEAR__dateCreated"] <> "") {
			...
			if (is_numeric($this->YEAR__dateCreated->QueryStringValue)) {
				$_SESSION["sel_JobPosition_Vs_JobOffered_Candidates_YEAR__dateCreated"] = $this->YEAR__dateCreated->QueryStringValue;
				$this->ResetPager();
			}
		}
		$this->YEAR__dateCreated->SelectionList = @$_SESSION["sel_JobPosition_Vs_JobOffered_Candidates_YEAR__dateCreated"];
	...
	function CheckFilter() {
		...
		// Year Filter
		if (@$_SESSION["sel_JobPosition_Vs_JobOffered_Candidates_YEAR__dateCreated"] <> "")
			return TRUE;
		return FALSE;
	}
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_YEAR__dateCreated", $filter)) {
			...
			$_SESSION["sel_JobPosition_Vs_JobOffered_Candidates_YEAR__dateCreated"] = $ar;
		}
	}
...
// Create page object
if (!isset($JobPosition_Vs_JobOffered_Candidates_crosstab)) $JobPosition_Vs_JobOffered_Candidates_crosstab = new crJobPosition_Vs_JobOffered_Candidates_crosstab();
if (isset($Page)) $OldPage = $Page;
$Page = &$JobPosition_Vs_JobOffered_Candidates_crosstab;
...
// Create page object
var JobPosition_Vs_JobOffered_Candidates_crosstab = new ewr_Page("JobPosition_Vs_JobOffered_Candidates_crosstab");

// Page properties
JobPosition_Vs_JobOffered_Candidates_crosstab.PageID = "crosstab"; // Page ID
var EWR_PAGE_ID = JobPosition_Vs_JobOffered_Candidates_crosstab.PageID;

// Extend page with Chart_Rendering function
JobPosition_Vs_JobOffered_Candidates_crosstab.Chart_Rendering = 
...
JobPosition_Vs_JobOffered_Candidates_crosstab.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var fJobPosition_Vs_JobOffered_Candidatescrosstab = new ewr_Form("fJobPosition_Vs_JobOffered_Candidatescrosstab");

// Validate method
fJobPosition_Vs_JobOffered_Candidatescrosstab.Validate = function() {
...
fJobPosition_Vs_JobOffered_Candidatescrosstab.Form_CustomValidate = 
...
fJobPosition_Vs_JobOffered_Candidatescrosstab.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fJobPosition_Vs_JobOffered_Candidatescrosstab.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fJobPosition_Vs_JobOffered_Candidatescrosstab" id="fJobPosition_Vs_JobOffered_Candidatescrosstab" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fJobPosition_Vs_JobOffered_Candidatescrosstab_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<div id="c_YEAR__dateCreated" class="ewCell form-group">
	...
	<select id="YEAR__dateCreated" class="form-control" name="YEAR__dateCreated" onchange="ewrForms['fJobPosition_Vs_JobOffered_Candidatescrosstab'].Submit();">
<?php
...
<script type="text/javascript">
fJobPosition_Vs_JobOffered_Candidatescrosstab.Init();
fJobPosition_Vs_JobOffered_Candidatescrosstab.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
<?php } ?>
...
<?php include "JobPosition_Vs_JobOffered_Candidatesctbpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php if ($Page->GrpFldCount > 0) { ?>
		<td class="ewRptColSummary" colspan="<?php echo $Page->GrpFldCount ?>"><div><?php echo $Page->Job_Offered_Candidates->FldCaption() ?>&nbsp;(<?php echo $ReportLanguage->Phrase("RptSum") ?>)&nbsp;</div></td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Job_Name">
		<div class="JobPosition_Vs_JobOffered_Candidates_Job_Name"><span class="ewTableHeaderCaption"><?php echo $Page->Job_Name->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="Job_Name">
<?php if ($Page->SortUrl($Page->Job_Name) == "") { ?>
		<div class="ewTableHeaderBtn ewPointer JobPosition_Vs_JobOffered_Candidates_Job_Name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Job_Name) ?>',0);">
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_JobPosition_Vs_JobOffered_Candidates_Job_Name"<?php echo $Page->Job_Name->ViewAttributes() ?>><?php echo $Page->Job_Name->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
...
<?php } ?>
	<!-- Right slot -->
<a id="cht_JobPosition_Vs_JobOffered_Candidates_C_job_position_vs_job_offered"></a>
<div class="">
<div id="div_ctl_JobPosition_Vs_JobOffered_Candidates_C_job_position_vs_job_offered" class="ewChart">
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<div class="ewChartSort">
<form class="ewForm form-horizontal" action="<?php echo ewr_CurrentPage() ?>#cht_JobPosition_Vs_JobOffered_Candidates_C_job_position_vs_job_offered">
...
<option value="1"<?php if ($Page->C_job_position_vs_job_offered->ChartSortType == "1") echo " selected=\"selected\"" ?>><?php echo $ReportLanguage->Phrase("ChartOrderXAsc") ?></option>
<option value="2"<?php if ($Page->C_job_position_vs_job_offered->ChartSortType == "2") echo " selected=\"selected\"" ?>><?php echo $ReportLanguage->Phrase("ChartOrderXDesc") ?></option>
<option value="3"<?php if ($Page->C_job_position_vs_job_offered->ChartSortType == "3") echo " selected=\"selected\"" ?>><?php echo $ReportLanguage->Phrase("ChartOrderYAsc") ?></option>
<option value="4"<?php if ($Page->C_job_position_vs_job_offered->ChartSortType == "4") echo " selected=\"selected\"" ?>><?php echo $ReportLanguage->Phrase("ChartOrderYDesc") ?></option>
</select>
<input type="hidden" id="chartorder" name="chartorder" value="C_job_position_vs_job_offered">
</form>
</div>
<?php } ?>
<div id="div_JobPosition_Vs_JobOffered_Candidates_C_job_position_vs_job_offered" class="ewChartDiv"></div>
<!-- grid component -->
<div id="div_JobPosition_Vs_JobOffered_Candidates_C_job_position_vs_job_offered_grid" class="ewChartGrid"></div>
</div>
</div>
<?php

// Set up chart object
$Chart = &$Table->C_job_position_vs_job_offered;
...
?>
<?php include_once "JobPosition_Vs_JobOffered_Candidates_C_job_position_vs_job_offeredchart.php" ?>
<?php if ($Page->Export == "") { ?>
	</div>
	<!-- right container (end) -->
<div class="clearfix"></div>
