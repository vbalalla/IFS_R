...
<?php include_once "Number_of_Interviews_Vs_Job_Positionsctbinfo.php" ?>
<?php
...
$Number_of_Interviews_Vs_Job_Positions_crosstab = NULL; // Initialize page object first

class crNumber_of_Interviews_Vs_Job_Positions_crosstab extends crNumber_of_Interviews_Vs_Job_Positions {
	...
	// Page object name
	var $PageObjName = 'Number_of_Interviews_Vs_Job_Positions_crosstab';
	...
	function __construct() {
		...
		// Table object (Number_of_Interviews_Vs_Job_Positions)
		if (!isset($GLOBALS["Number_of_Interviews_Vs_Job_Positions"])) {
			$GLOBALS["Number_of_Interviews_Vs_Job_Positions"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Number_of_Interviews_Vs_Job_Positions"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Number of Interviews Vs Job Positions', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fNumber_of_Interviews_Vs_Job_Positionscrosstab";
	}
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_Number_of_Interviews_Vs_Job_Positions\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Number_of_Interviews_Vs_Job_Positions',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fNumber_of_Interviews_Vs_Job_Positionscrosstab\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fNumber_of_Interviews_Vs_Job_Positionscrosstab\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fNumber_of_Interviews_Vs_Job_Positionscrosstab\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'Number_of_Interviews_Vs_Job_Positions_jbName') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_YEAR__dateCreated"] = "";
		...
		if (is_array(@$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_jbName"])) {
			$this->LoadSelectionFromSession('jbName');
		} elseif (@$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_jbName"] == EWR_INIT_VALUE) { // Select all
			$this->jbName->SelectionList = "";
		}
		...
		if (@$_GET["YEAR__dateCreated"] <> "") {
			...
			if (is_numeric($this->YEAR__dateCreated->QueryStringValue)) {
				$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_YEAR__dateCreated"] = $this->YEAR__dateCreated->QueryStringValue;
				$this->ResetPager();
			}
		}
		$this->YEAR__dateCreated->SelectionList = @$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_YEAR__dateCreated"];
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'Number_of_Interviews_Vs_Job_Positions_jbName')
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_Number_of_Interviews_Vs_Job_Positions_jbName'] = ($this->jbName->SelectionList == "") ? EWR_INIT_VALUE : $this->jbName->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_Number_of_Interviews_Vs_Job_Positions_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Number_of_Interviews_Vs_Job_Positions_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Number_of_Interviews_Vs_Job_Positions_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Number_of_Interviews_Vs_Job_Positions_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_Number_of_Interviews_Vs_Job_Positions_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_Number_of_Interviews_Vs_Job_Positions_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_Number_of_Interviews_Vs_Job_Positions_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_Number_of_Interviews_Vs_Job_Positions_' . $parm] = $sv;
		$_SESSION['so_Number_of_Interviews_Vs_Job_Positions_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Number_of_Interviews_Vs_Job_Positions_' . $parm] = $sv1;
		$_SESSION['so_Number_of_Interviews_Vs_Job_Positions_' . $parm] = $so1;
		$_SESSION['sc_Number_of_Interviews_Vs_Job_Positions_' . $parm] = $sc;
		$_SESSION['sv2_Number_of_Interviews_Vs_Job_Positions_' . $parm] = $sv2;
		$_SESSION['so2_Number_of_Interviews_Vs_Job_Positions_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_$parm"] = "";
		$_SESSION["rf_Number_of_Interviews_Vs_Job_Positions_$parm"] = "";
		$_SESSION["rt_Number_of_Interviews_Vs_Job_Positions_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Number_of_Interviews_Vs_Job_Positions_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Number_of_Interviews_Vs_Job_Positions_$parm"];
	}
	...
	function CheckFilter() {
		...
		// Year Filter
		if (@$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_YEAR__dateCreated"] <> "")
			return TRUE;
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_YEAR__dateCreated", $filter)) {
			...
			$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_YEAR__dateCreated"] = $ar;
		}
		...
		if (array_key_exists("sel_jbName", $filter)) {
			...
			$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_jbName"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "jbName");
			$this->jbName->SelectionList = "";
			$_SESSION["sel_Number_of_Interviews_Vs_Job_Positions_jbName"] = "";
		}
	}
...
// Create page object
if (!isset($Number_of_Interviews_Vs_Job_Positions_crosstab)) $Number_of_Interviews_Vs_Job_Positions_crosstab = new crNumber_of_Interviews_Vs_Job_Positions_crosstab();
if (isset($Page)) $OldPage = $Page;
$Page = &$Number_of_Interviews_Vs_Job_Positions_crosstab;
...
// Create page object
var Number_of_Interviews_Vs_Job_Positions_crosstab = new ewr_Page("Number_of_Interviews_Vs_Job_Positions_crosstab");

// Page properties
Number_of_Interviews_Vs_Job_Positions_crosstab.PageID = "crosstab"; // Page ID
var EWR_PAGE_ID = Number_of_Interviews_Vs_Job_Positions_crosstab.PageID;

// Extend page with Chart_Rendering function
Number_of_Interviews_Vs_Job_Positions_crosstab.Chart_Rendering = 
...
Number_of_Interviews_Vs_Job_Positions_crosstab.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fNumber_of_Interviews_Vs_Job_Positionscrosstab = new ewr_Form("fNumber_of_Interviews_Vs_Job_Positionscrosstab");

// Validate method
fNumber_of_Interviews_Vs_Job_Positionscrosstab.Validate = function() {
...
fNumber_of_Interviews_Vs_Job_Positionscrosstab.Form_CustomValidate = 
...
fNumber_of_Interviews_Vs_Job_Positionscrosstab.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fNumber_of_Interviews_Vs_Job_Positionscrosstab.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fNumber_of_Interviews_Vs_Job_Positionscrosstab" id="fNumber_of_Interviews_Vs_Job_Positionscrosstab" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fNumber_of_Interviews_Vs_Job_Positionscrosstab_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="Number_of_Interviews_Vs_Job_Positions" data-field="x_jbName" id="sv_jbName" name="sv_jbName" size="30" maxlength="30" placeholder="<?php echo $Page->jbName->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->jbName->SearchValue) ?>"<?php echo $Page->jbName->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fNumber_of_Interviews_Vs_Job_Positionscrosstab.Init();
fNumber_of_Interviews_Vs_Job_Positionscrosstab.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "Number_of_Interviews_Vs_Job_Positionsctbpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
</div>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="jbName">
		<div class="Number_of_Interviews_Vs_Job_Positions_jbName"><span class="ewTableHeaderCaption"><?php echo $Page->jbName->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="jbName">
<?php if ($Page->SortUrl($Page->jbName) == "") { ?>
		<div class="ewTableHeaderBtn ewPointer Number_of_Interviews_Vs_Job_Positions_jbName" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->jbName) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->jbName->FldCaption() ?></span>			
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Number_of_Interviews_Vs_Job_Positions_jbName', false, '<?php echo $Page->jbName->RangeFrom; ?>', '<?php echo $Page->jbName->RangeTo; ?>');" id="x_jbName"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Number_of_Interviews_Vs_Job_Positions_jbName" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->jbName) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->jbName->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_Number_of_Interviews_Vs_Job_Positions_jbName"<?php echo $Page->jbName->ViewAttributes() ?>><?php echo $Page->jbName->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
...
<?php } ?>
	<!-- Bottom slot -->
<a id="cht_Number_of_Interviews_Vs_Job_Positions_Number_of_Interviews_Vs_Job_Positions"></a>
<div class="">
<div id="div_ctl_Number_of_Interviews_Vs_Job_Positions_Number_of_Interviews_Vs_Job_Positions" class="ewChart">
<div id="div_Number_of_Interviews_Vs_Job_Positions_Number_of_Interviews_Vs_Job_Positions" class="ewChartDiv"></div>
<!-- grid component -->
<div id="div_Number_of_Interviews_Vs_Job_Positions_Number_of_Interviews_Vs_Job_Positions_grid" class="ewChartGrid"></div>
</div>
</div>
<?php

// Set up chart object
$Chart = &$Table->Number_of_Interviews_Vs_Job_Positions;
...
?>
<?php include_once "Number_of_Interviews_Vs_Job_Positions_Number_of_Interviews_Vs_Job_Positionschart.php" ?>
